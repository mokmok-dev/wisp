#![allow(clippy::inline_always, clippy::ref_as_ptr)] // Generated Windows COM glue.
...
use wasapi::{BufferInfo, DeviceEnumerator, Direction, SampleType, StreamMode, WaveFormat};
...
#[derive(Clone, Copy)]
struct CaptureClock {
    origin_qpc_100ns: u64,
    fallback_origin: Instant,
}

impl CaptureClock {
    fn now() -> std::result::Result<Self, String> {
        let mut counter = 0_i64;
        let mut frequency = 0_i64;
        // SAFETY: both APIs only initialize the valid out-pointers supplied
        // here and have no lifetime requirements.
        unsafe {
            QueryPerformanceCounter(&raw mut counter).map_err(|error| error.to_string())?;
            QueryPerformanceFrequency(&raw mut frequency).map_err(|error| error.to_string())?;
        }
        if counter < 0 || frequency <= 0 {
            return Err("QueryPerformanceCounter returned an invalid value".into());
        }
        let origin_qpc_100ns = u64::try_from(
            (u128::try_from(counter).map_err(|error| error.to_string())? * 10_000_000)
                / u128::try_from(frequency).map_err(|error| error.to_string())?,
        )
        .map_err(|error| error.to_string())?;
        Ok(Self {
            origin_qpc_100ns,
            fallback_origin: Instant::now(),
        })
    }
    ...
    fn packet_timestamp(
        ...
        packet_timestamp_from_qpc(
            self.origin_qpc_100ns,
            info.qpc_100ns,
            info.timestamp_error,
            self.fallback_origin.elapsed(),
            frames_to_duration(packet_frames, WASAPI_SAMPLE_RATE),
        )
    }
}

fn packet_timestamp_from_qpc(
    origin_qpc_100ns: u64,
    packet_qpc_100ns: u64,
    timestamp_error: bool,
    fallback_now: Duration,
    packet_duration: Duration,
) -> Duration {
    let fallback = fallback_now.saturating_sub(packet_duration);
    if !qpc_timestamp_is_valid(
        origin_qpc_100ns,
        packet_qpc_100ns,
        timestamp_error,
        fallback,
    ...
    let candidate = Duration::from_nanos(
        packet_qpc_100ns
            .saturating_sub(origin_qpc_100ns)
            .saturating_mul(100),
    );
    // A corrupt driver timestamp must not create hours of silence padding.
    // Permit ordinary scheduling latency but reject values outside a bounded
    // window around this process's monotonic clock.
    candidate
}

fn qpc_timestamp_is_valid(
    origin_qpc_100ns: u64,
    packet_qpc_100ns: u64,
    timestamp_error: bool,
    fallback_timestamp: Duration,
) -> bool {
    const QPC_TOLERANCE: Duration = Duration::from_secs(5);
    if timestamp_error || packet_qpc_100ns < origin_qpc_100ns {
        return false;
    }
    let candidate = Duration::from_nanos(
        packet_qpc_100ns
    ...
    candidate >= fallback_timestamp.saturating_sub(QPC_TOLERANCE)
        && candidate <= fallback_timestamp.saturating_add(QPC_TOLERANCE)
}
...
impl IMMNotificationClient_Impl for DefaultEndpointNotification_Impl {
    ...
    fn OnDefaultDeviceChanged(
        ...
        if matches_default_endpoint_change(flow, role, self.flow) {
...
fn matches_default_endpoint_change(
    flow: EDataFlow,
    role: ERole,
    expected: EDataFlow,
) -> bool {
    flow == expected && role == eConsole
}

struct DefaultEndpointRegistration {
    enumerator: IMMDeviceEnumerator,
    callback: IMMNotificationClient,
    changed: Arc<AtomicBool>,
}

impl DefaultEndpointRegistration {
    fn new(source: SourceLabel) -> std::result::Result<Self, String> {
        let flow = match source {
            SourceLabel::Mic => eCapture,
            SourceLabel::System => eRender,
        };
        let enumerator: IMMDeviceEnumerator =
            unsafe { CoCreateInstance(&MMDeviceEnumerator, None, CLSCTX_ALL) }
                .map_err(|error| error.to_string())?;
        let changed = Arc::new(AtomicBool::new(false));
        let callback: IMMNotificationClient = DefaultEndpointNotification {
            flow,
...
impl Drop for DefaultEndpointRegistration {
    fn drop(&mut self) {
        let _ = unsafe {
            self.enumerator
...
impl From<&BufferInfo> for PacketMetadata {
    fn from(info: &BufferInfo) -> Self {
        Self {
            device_position: info.index,
            qpc_100ns: info.timestamp,
...
pub struct WasapiRecording {
    ...
    fatal_error_receiver: channel::Receiver<RecordingNotification>,
    warning_receiver: channel::Receiver<RecordingNotification>,
    mic_path: PathBuf,
    system_path: PathBuf,
}
...
pub(crate) enum RecordingNotification {
    Fatal(String),
    Warning(String),
}

struct RecordingNotificationSenders {
    fatal: channel::Sender<RecordingNotification>,
    warning: channel::Sender<RecordingNotification>,
}

struct RecordingNotificationReceivers {
    fatal: channel::Receiver<RecordingNotification>,
    warning: channel::Receiver<RecordingNotification>,
}
...
impl WasapiCapture {
    ...
    pub fn start() -> Result<Self> {
        ...
        let capture_clock = CaptureClock::now().map_err(|error| {
            SessionError::Start(format!("failed to initialize QPC clock: {error}"))
        })?;
    ...
    /// Stop both streams and report worker panics.
    ///
    /// This is the observable counterpart to the source-compatible
    /// best-effort [`Self::stop`].
    /// # Errors
    ///
    /// Returns an error if a capture worker panicked while it was being
    /// joined.
    pub fn stop_checked(&mut self) -> Result<()> {
        self.stop_requested.store(true, Ordering::SeqCst);
        join_workers(&mut self.workers).map_err(SessionError::Start)
    }
}
...
impl WasapiRecording {
    ...
    pub fn try_recv_error(&self) -> Option<String> {
        self.try_recv_fatal()
            .or_else(|| self.try_recv_warning())
            .map(|notification| match notification {
                RecordingNotification::Fatal(message) | RecordingNotification::Warning(message) => {
                    message
                },
            })
    }
    ...
    pub(crate) const fn fatal_error_receiver(&self) -> &channel::Receiver<RecordingNotification> {
        &self.fatal_error_receiver
    }

    pub(crate) const fn warning_receiver(&self) -> &channel::Receiver<RecordingNotification> {
        &self.warning_receiver
    }

    pub(crate) fn try_recv_fatal(&self) -> Option<RecordingNotification> {
        self.fatal_error_receiver.try_recv().ok()
    }

    pub(crate) fn try_recv_warning(&self) -> Option<RecordingNotification> {
        self.warning_receiver.try_recv().ok()
    }

    /// Detect workers that exited without publishing their normal terminal
    /// notification. This closes the gap left by a thread panic, since a
    /// panicking producer cannot send through the capture control channel.
    pub(crate) fn detect_terminal_failure(&self) -> Option<SessionError> {
        let capture_exited = self.capture.lock().map_or(true, |capture| {
            !capture.stop_requested.load(Ordering::SeqCst)
                && capture.workers.iter().any(JoinHandle::is_finished)
        });
        let writer_exited = self.writer.lock().map_or(true, |writer| {
            matches!(
                &*writer,
                RecordingWriterState::Active(handle) if handle.is_finished()
            )
        });
        if !capture_exited && !writer_exited {
            return None;
        }

        Some(SessionError::Start(if capture_exited {
            "WASAPI capture worker exited unexpectedly".into()
        } else {
            "Ogg writer exited unexpectedly".into()
        }))
    }
    ...
    pub fn stop(&self) -> Result<()> {
        ...
        let capture_result = capture.stop_checked();
        drop(capture);
        ...
        let result = combine_session_results(capture_result, writer_result);
...
fn run_recording_worker(
    ...
    if let Err(error) = &result {
        ...
        let _ = notifications
            .fatal
            .send(RecordingNotification::Fatal(error.to_string()));
    }
    result
}

#[allow(clippy::too_many_lines)] // Event variants are kept together to preserve ordering.
fn recording_loop(
    receiver: &CaptureEventReceiver,
    ...
    let processing_result = (|| {
                ...
                CaptureEvent::Samples(frame) => {
                    ...
                    match frame.track_id() {
                        ...
                        TrackId::SYSTEM => {
                            ...
                            let written_gap =
                                push_silence(&mut system, gap.frames, abort_requested)?;
                            system.push(samples)?;
                ...
                CaptureEvent::Overflow {
                    ...
                    match track_id {
                        TrackId::MICROPHONE => {
                            ...
                            let written =
                                push_silence(&mut mic, compensation.frames, abort_requested)?;
        ...
        // shorter stream preserves mic/system alignment when one endpoint
        // starts later, goes idle, or stops producing first.
...
fn finish_both_recorders(
    mic: &mut OggOpusRecorder,
    system: &mut OggOpusRecorder,
) -> io::Result<()> {
    run_both_finalizers(|| mic.finish(), || system.finish())
}

fn run_both_finalizers(
    mut mic: impl FnMut() -> io::Result<()>,
    mut system: impl FnMut() -> io::Result<()>,
) -> io::Result<()> {
    aggregate_finish_results(mic(), system())
}

fn aggregate_finish_results(
    mic: io::Result<()>,
    system: io::Result<()>,
) -> io::Result<()> {
    let mut errors = Vec::new();
    if let Err(error) = mic {
        errors.push(format!("microphone Ogg finalization failed: {error}"));
    }
    if let Err(error) = system {
        errors.push(format!("system Ogg finalization failed: {error}"));
    }
    if errors.is_empty() {
        Ok(())
    } else {
...
fn combine_session_results(
    ...
    match (first, second) {
        (Ok(()), Ok(())) => Ok(()),
        (Err(error), Ok(())) | (Ok(()), Err(error)) => Err(error),
        (Err(first), Err(second)) => Err(SessionError::Start(format!("{first}; {second}"))),
    }
}
...
#[allow(clippy::too_many_lines)] // Startup and COM teardown must remain in one worker scope.
fn run_capture(
    source: SourceLabel,
    event_sender: &RealtimeCaptureSender,
    ...
    // Register before resolving the default endpoint so a switch cannot occur
    // unnoticed between selection and notification registration.
    let endpoint_registration = DefaultEndpointRegistration::new(source)?;
    ...
    let mut expected_device_position = None;
    let mut invalid_qpc_reported = false;
    while !stop_requested.load(Ordering::SeqCst) {
        if endpoint_registration.has_changed() {
            return Err(format!(
                "{} default audio endpoint changed during recording",
                source_name(source)
            ));
        }
        ...
        if !event_handle
            .wait_for_event_timeout(EVENT_WAIT_MILLIS)
            .map_err(|error| format!("WASAPI event wait failed: {error}"))?
        {
            continue;
        }
        drain_available_packets(
            ...
            |packet_frames| {
                let byte_len = usize::try_from(packet_frames)
                    .map_err(|error| error.to_string())?
                    .checked_mul(size_of::<f32>())
                    .ok_or_else(|| "WASAPI packet byte length overflow".to_owned())?;
                let mut bytes = vec![0_u8; byte_len];
                let (read_frames, info) = capture_client
                    .read_from_device(&mut bytes)
                    .map_err(|error| error.to_string())?;
                bytes.truncate(
                    usize::try_from(read_frames)
                        .map_err(|error| error.to_string())?
                        .saturating_mul(size_of::<f32>()),
                );
                Ok((read_frames, PacketMetadata::from(&info), bytes))
            },
            |packet_frames, metadata, bytes| {
                let mut bytes = VecDeque::from(bytes);
                let mut samples = if metadata.silent {
                    vec![0.0; usize::try_from(packet_frames).map_err(|error| error.to_string())?]
                } else {
                    drain_f32_samples(&mut bytes)
                };
                if source == SourceLabel::Mic && microphone_muted.load(Ordering::SeqCst) {
                    samples.fill(0.0);
                }
                let packet_frames = u64::from(packet_frames);
                let position_gap = expected_device_position.map_or(0, |expected: u64| {
                    metadata.device_position.saturating_sub(expected)
                });
                if metadata.data_discontinuity || position_gap > 0 {
                    let _ = event_sender.send_control(CaptureControlEvent::Error {
                        track_id: Some(source.track_id()),
                        message: format!(
                            "WASAPI data discontinuity (device position {}, expected {:?}, missing {position_gap} frames)",
                            metadata.device_position, expected_device_position
                        ),
                        recoverable: true,
                    });
                }
                expected_device_position =
                    Some(metadata.device_position.saturating_add(packet_frames));
                if !invalid_qpc_reported
                    && !qpc_timestamp_is_valid(
                        capture_clock.origin_qpc_100ns,
                        metadata.qpc_100ns,
                        metadata.timestamp_error,
                        capture_clock.fallback_origin.elapsed(),
                    )
                {
                    invalid_qpc_reported = true;
                    let _ = event_sender.send_control(CaptureControlEvent::Error {
                        track_id: Some(source.track_id()),
                        message:
                            "WASAPI supplied an invalid QPC timestamp; using monotonic fallback"
                                .into(),
                        recoverable: true,
                    });
                }
                let frame = AudioFrame::from_f32(
                    ...
                    source.source_kind(),
                    sequence,
                    MonotonicTimestamp::from_duration(
                        capture_clock.packet_timestamp(&metadata, packet_frames),
                    ),
                    WASAPI_SAMPLE_RATE,
                    WASAPI_CHANNELS,
                    samples,
...
fn drain_available_packets<Next, Read, Process>(
    mut next_packet_size: Next,
    mut read_packet: Read,
    mut process_packet: Process,
) -> std::result::Result<usize, String>
where
    Next: FnMut() -> std::result::Result<Option<u32>, String>,
    Read: FnMut(u32) -> std::result::Result<(u32, PacketMetadata, Vec<u8>), String>,
    Process: FnMut(u32, PacketMetadata, Vec<u8>) -> std::result::Result<(), String>,
{
    let mut drained = 0_usize;
    loop {
        let Some(available_frames) = next_packet_size()? else {
            return Err("WASAPI shared capture did not report packet sizes".into());
        };
        if available_frames == 0 {
            return Ok(drained);
        }
        let (read_frames, metadata, bytes) = read_packet(available_frames)?;
        if read_frames == 0 {
            return Err("WASAPI reported a non-empty packet but returned zero frames".into());
        }
        if read_frames != available_frames {
            return Err(format!(
                "WASAPI packet size changed while reading (advertised {available_frames}, read {read_frames})"
            ));
        }
        let expected_bytes = usize::try_from(read_frames)
            .map_err(|error| error.to_string())?
            .checked_mul(size_of::<f32>())
            .ok_or_else(|| "WASAPI packet byte length overflow".to_owned())?;
        if bytes.len() != expected_bytes {
            return Err(format!(
                "WASAPI packet payload length mismatch (expected {expected_bytes}, got {})",
                bytes.len()
            ));
        }
        process_packet(read_frames, metadata, bytes)?;
        drained = drained.saturating_add(1);
    }
}

fn join_workers(workers: &mut Vec<JoinHandle<()>>) -> std::result::Result<(), String> {
    ...
    if panics == 0 {
        ...
        Err(format!("{panics} WASAPI capture worker(s) panicked"))
    }
}
...
mod tests {
    ...
    use super::{
        CaptureClock, FORCE_STARTUP_REAPER_SPAWN_FAILURE, NOTIFICATION_QUEUE_CAPACITY,
        PacketMetadata, RecordingNotification, Startup, TrackTimeline, WasapiCaptureEvent,
        WasapiPcmChunk, aggregate_finish_results, await_capture_startup, drain_available_packets,
        drain_f32_samples, ensure_startup_reaper, join_workers, legacy_capture_event,
        matches_default_endpoint_change, packet_timestamp_from_qpc, publish_worker_failure,
        push_silence, push_silence_chunks, qpc_timestamp_is_valid, reap_finished_startup_workers,
        recording_loop, recording_notification_channels, recv_recording_notification,
        recv_recording_notification_with_wait_hook, reserve_startup_attempt,
        retain_startup_workers, run_both_finalizers, run_recording_worker, startup_worker_registry,
    };
    ...
    #[test]
    fn event_wake_drains_every_available_packet() {
        let mut sizes = VecDeque::from([Some(3), Some(2), Some(0)]);
        ...
        let drained = drain_available_packets(
            || Ok(sizes.pop_front().unwrap()),
            |frames| Ok((frames, metadata, vec![0; frames as usize * 4])),
            |frames, _, _| {
    ...
    #[test]
    fn packet_drain_rejects_advertised_and_payload_mismatches() {
        let metadata = PacketMetadata {
            device_position: 0,
            qpc_100ns: 0,
            silent: false,
        ...
        let changed = drain_available_packets(
            || Ok(Some(3)),
            |_| Ok((2, metadata, vec![0; 8])),
            |_, _, _| Ok(()),
        )
        .unwrap_err();
        assert!(changed.contains("advertised 3, read 2"));

        let short = drain_available_packets(
            || Ok(Some(2)),
            |_| Ok((2, metadata, vec![0; 4])),
            |_, _, _| Ok(()),
        )
        .unwrap_err();
        assert!(short.contains("payload length mismatch"));
    }

    #[test]
    fn packet_drain_propagates_all_terminal_boundaries() {
        let metadata = PacketMetadata {
            device_position: 0,
            qpc_100ns: 0,
            silent: false,
        ...
        let unsupported =
            drain_available_packets(|| Ok(None), |_| unreachable!(), |_, _, _| unreachable!())
                .unwrap_err();
        assert!(unsupported.contains("did not report packet sizes"));

        let zero_read = drain_available_packets(
            || Ok(Some(1)),
            |_| Ok((0, metadata, Vec::new())),
            |_, _, _| unreachable!(),
        )
        .unwrap_err();
        assert!(zero_read.contains("returned zero frames"));

        let next_error = drain_available_packets(
            || Err("next fault".into()),
            |_| unreachable!(),
            |_, _, _| unreachable!(),
        )
        .unwrap_err();
        assert_eq!(next_error, "next fault");

        let read_error = drain_available_packets(
            || Ok(Some(1)),
            |_| Err("read fault".into()),
            |_, _, _| unreachable!(),
        )
        .unwrap_err();
        assert_eq!(read_error, "read fault");

        let process_error = drain_available_packets(
            || Ok(Some(1)),
            |_| Ok((1, metadata, vec![0; 4])),
            |_, _, _| Err("process fault".into()),
        )
        .unwrap_err();
        assert_eq!(process_error, "process fault");
    }

    #[test]
    fn packet_metadata_flags_and_qpc_timestamp_drive_capture_timeline() {
        let clock = CaptureClock {
            origin_qpc_100ns: 1_000,
            fallback_origin: Instant::now(),
        };
        let valid = PacketMetadata {
            device_position: 80,
            qpc_100ns: 201_000,
            silent: true,
        ...
        assert!(valid.silent);
        assert!(valid.data_discontinuity);
        assert_eq!(
            clock.packet_timestamp(&valid, 160),
            Duration::from_millis(20)
        );

        let invalid = PacketMetadata {
            timestamp_error: true,
            ..valid
        };
        assert!(clock.packet_timestamp(&invalid, 160) < Duration::from_millis(20));
    }

    #[test]
    fn qpc_timestamp_validation_is_deterministic() {
        let fallback_now = Duration::from_secs(10);
        let packet_duration = Duration::from_millis(10);
        let near_fallback_qpc = 1_000 + 99_900_000;
        assert_eq!(
            packet_timestamp_from_qpc(
                1_000,
                near_fallback_qpc,
        ...
        assert_eq!(
            packet_timestamp_from_qpc(1_000, 999, false, fallback_now, packet_duration),
            Duration::from_millis(9_990)
        );
        assert_eq!(
            packet_timestamp_from_qpc(1_000, 201_000, false, fallback_now, packet_duration),
            Duration::from_millis(9_990)
        );
        assert_eq!(
            packet_timestamp_from_qpc(1_000, 200_000_000, false, fallback_now, packet_duration),
            Duration::from_millis(9_990)
        );
        assert_eq!(
            packet_timestamp_from_qpc(
        ...
        let lower_boundary_qpc = 1_000 + 49_900_000;
        let upper_boundary_qpc = 1_000 + 149_900_000;
        assert!(qpc_timestamp_is_valid(
            1_000,
            lower_boundary_qpc,
            false,
            Duration::from_millis(9_990),
    ...
    #[test]
    fn endpoint_notification_matches_only_selected_console_flow() {
        use windows::Win32::Media::Audio::{eCapture, eCommunications, eConsole, eRender};

        assert!(matches_default_endpoint_change(
            eCapture, eConsole, eCapture
        ));
        assert!(!matches_default_endpoint_change(
    ...
    #[test]
    fn both_track_finish_errors_are_aggregated() {
        let error = aggregate_finish_results(
            Err(std::io::Error::other("mic fault")),
            Err(std::io::Error::other("system fault")),
        )
        .unwrap_err()
        .to_string();

        assert!(error.contains("microphone Ogg finalization failed: mic fault"));
        assert!(error.contains("system Ogg finalization failed: system fault"));
    }

    #[test]
    fn one_track_finish_failure_does_not_hide_the_other_result() {
        let error = aggregate_finish_results(Err(std::io::Error::other("mic only")), Ok(()))
            .unwrap_err()
            .to_string();
        assert!(error.contains("mic only"));
        assert!(!error.contains("system Ogg finalization failed"));
    }

    #[test]
    fn both_finalizers_run_even_when_the_first_fails() {
        let mic_calls = std::cell::Cell::new(0);
        let system_calls = std::cell::Cell::new(0);
        let error = run_both_finalizers(
            || {
                mic_calls.set(mic_calls.get() + 1);
                Err(std::io::Error::other("mic fault"))
            },
            || {
        ...
        .unwrap_err()
        .to_string();
        assert_eq!(mic_calls.get(), 1);
        assert_eq!(system_calls.get(), 1);
        assert!(error.contains("mic fault"));
        assert!(error.contains("system fault"));
    }

    #[test]
    fn capture_worker_panics_are_terminal_join_errors() {
        let mut workers = vec![std::thread::spawn(|| panic!("worker fault"))];
        assert!(join_workers(&mut workers).unwrap_err().contains("panicked"));
    ...
    fn fatal_notification_has_reserved_capacity_when_warning_queue_is_full() {
        ...
        for index in 0..NOTIFICATION_QUEUE_CAPACITY {
            senders
                .warning
                .try_send(RecordingNotification::Warning(format!("warning {index}")))
                .unwrap();
        }
        assert!(
            senders
                .warning
                .try_send(RecordingNotification::Warning("another warning".into()))
                .is_err()
        );

        senders
            .fatal
            .send(RecordingNotification::Fatal("recording failed".into()))
            .unwrap();
        assert_eq!(
            receivers.fatal.try_recv().unwrap(),
            RecordingNotification::Fatal("recording failed".into())
        );
    }
    ...
    fn blocking_notification_wait_rechecks_fatal_before_warning() {
        ...
        senders
            .warning
            .send(RecordingNotification::Warning("warning".into()))
            .unwrap();
        senders
            .fatal
            .send(RecordingNotification::Fatal("fatal".into()))
            .unwrap();
        release.wait();

        assert_eq!(
            waiter.join().unwrap(),
            Some(RecordingNotification::Fatal("fatal".into()))
        );
    }
    ...
    fn recording_worker_delivers_fatal_when_warning_queue_is_full() {
        ...
        assert!(matches!(
            ...
            RecordingNotification::Fatal(message)
                if message.contains("unsupported WASAPI recording format")
        ));
    }
