...
struct WispCtl {
    ...
    static func run() async throws {
        ...
        log("Wisp PoC v2 (mic + system audio) starting")

        // 1. Permissions: mic + speech recognition.
        // Screen Recording is requested implicitly by SCShareableContent.
        ...
        {
            log("Downloading ja-JP speech model...")
        ...
        log("[MIC] native format sr=\(micFormat.sampleRate) ch=\(micFormat.channelCount)")
        ...
        let systemCapture = ProcessTapCapture { buffer in
            ...
            Task {
                do {
                    ...
                    log("[SYS] failed to build pipeline: \(error)")
        ...
        log("Recording. Speak in Japanese — try playing a YouTube clip too. Ctrl+C to stop.")
        ...
        if let sysPipeline {
            ...
            log("[SYS] no audio was ever received (system was silent)")
        }

        log("Done.")
        log("  MIC WAV: \(micWavURL.path)")
        log("  SYS WAV: \(sysWavURL.path)")
    }
}
