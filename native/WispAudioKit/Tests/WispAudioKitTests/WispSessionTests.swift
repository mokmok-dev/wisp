...
final class WispSessionTests: XCTestCase {
  ...
  func testAnalyzerFormatAndConverterFailuresHonorPermissiveAndStrictPolicy() {
    ...
    for failure in failures {
      guard
        case .recordOnly(let message) = analyzerSetupDisposition(
  ...
  func testReentrantSynchronousBridgeMutationsReturnErrorsInsteadOfDeadlocking() throws {
    ...
    let error = try XCTUnwrap(
      wisp_session_last_error_message(
...
extension Data {
  fileprivate func readLittleEndianUInt32(at offset: Int) -> UInt32 {
    UInt32(self[offset])
  ...
  fileprivate var oggTestCRC: UInt32 {
    var crc: UInt32 = 0
    for byte in self {
      crc ^= UInt32(byte) << 24
      for _ in 0..<8 {
