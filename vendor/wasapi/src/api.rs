...
use windows::{
    ...
    Win32::Foundation::{HANDLE, WAIT_FAILED, WAIT_OBJECT_0, WAIT_TIMEOUT},
    Win32::Media::Audio::{
...
impl AudioCaptureClient {
    ...
    pub fn read_from_device(&self, data: &mut [u8]) -> WasapiRes<(u32, BufferInfo)> {
        ...
        if let Err(error) = copy_capture_payload(
    ...
    pub fn read_from_device_to_deque(&self, data: &mut VecDeque<u8>) -> WasapiRes<BufferInfo> {
        ...
        let len_in_bytes = nbr_frames_returned as usize * self.bytes_per_frame;
        let mut payload = vec![0; len_in_bytes];
        if let Err(error) =
            copy_capture_payload(&mut payload, buffer_ptr, buffer_info.flags.silent)
        {
            unsafe { self.client.ReleaseBuffer(nbr_frames_returned)? };
            return Err(error);
        }
...
impl Handle {
    /// Wait for an event and distinguish a normal timeout from a wait failure.
    pub fn wait_for_event_timeout(&self, timeout_ms: u32) -> WasapiRes<bool> {
        classify_wait_result(unsafe { WaitForSingleObject(self.handle, timeout_ms) }.0)
    }

    /// Wait for an event on a handle, with a timeout given in ms
    pub fn wait_for_event(&self, timeout_ms: u32) -> WasapiRes<()> {
        if !self.wait_for_event_timeout(timeout_ms)? {
...
fn classify_wait_result(result: u32) -> WasapiRes<bool> {
    ...
    debug_assert!(result == WAIT_FAILED.0 || result != WAIT_OBJECT_0.0);
    Err(WasapiError::Windows(windows_core::Error::from_thread()))
}

fn copy_capture_payload(
    destination: &mut [u8],
    source: *const u8,
    silent: bool,
) -> WasapiRes<()> {
    ...
    if source.is_null() {
        return Err(WasapiError::Windows(windows_core::Error::new(
            windows_core::HRESULT(0x8000_4003_u32 as i32),
            "WASAPI returned a null non-silent capture buffer",
        )));
    }
    let source = unsafe { slice::from_raw_parts(source, destination.len()) };
...
mod local_patch_tests {
    use super::{classify_wait_result, copy_capture_payload};
    use windows::Win32::Foundation::{WAIT_FAILED, WAIT_OBJECT_0, WAIT_TIMEOUT};

    #[test]
    fn silent_capture_never_reads_the_source_pointer() {
        let mut output = [0xff; 8];
        copy_capture_payload(&mut output, std::ptr::null(), true).unwrap();
        assert_eq!(output, [0; 8]);
    }

    #[test]
    fn non_silent_null_capture_buffer_is_rejected() {
        let mut output = [0; 8];
        assert!(copy_capture_payload(&mut output, std::ptr::null(), false).is_err());
    }

    #[test]
    fn wait_results_keep_timeout_distinct_from_failure() {
        assert_eq!(classify_wait_result(WAIT_OBJECT_0.0).unwrap(), true);
        assert_eq!(classify_wait_result(WAIT_TIMEOUT.0).unwrap(), false);
        assert!(classify_wait_result(WAIT_FAILED.0).is_err());
        assert!(classify_wait_result(17).is_err());
    }
}
